...
outlier_removal <- function(dat, by_column = c(1, 2), start_column = 3){
  ...
  for (i in 1:length(temp_outliers_residuals)) {
    ...
    temp_outliers_residuals[[i]] <- dat[rows, c(1,2, columns)]
    ...
    if(i==1){
      ...
      outlier_dat <- merge(outlier_dat, temp_outliers_residuals[[i]], by = colnames(dat)[by_column], all = TRUE)
    }
  }
